...
export class RlsMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    // болгосон).
    //
    // ⚠️ **Яг нэг зарчмаар (2 горимтой, ЗӨРЧИЛГҮЙ) тодорхойлбол:**
    // `originalEnd()` (жинхэнэ HTTP header/body бичих цэг) `flushed` flag
    // `true` болтол — өөрөөр хэлбэл `transactionSettled` (COMMIT/ROLLBACK)
    // СЕТТЛ хийж, ГАНЦ анхны flush бодитоор явагдтал — ЯГ НЭГ удаа л
    // дуудагддаг тул: (а) ТЭР НЭГ удаагийн flush-ийн ӨМНӨ ирсэн `res.end()`-ийн
    // дуудлага БҮР `pendingEndArgs`-г ДАХИН БИЧИЖ, тэдгээрийн ХАМГИЙН
    // СҮҮЛД ирсэн нь л бодитоор бичигдэнэ ("сүүлчийн бичилт ялна" —
    // RlsMiddleware-ийн success→`.catch()`-ийн алдааны зам хоёулаа яг ЭНЭ
    // нэг л удаагийн flush-ийн ӨМНӨ, Promise microtask дараалал баталгаатай
    // (алдааны зам үргэлж СҮҮЛД) ирдэг тул controller "амжилттай" гэж
    // эхлээд бодсон ч, транзакц эцэст нь бодитоор REJECT хийвэл клиент
    // ХУУРАМЧ 200 БИШ, ЖИНХЭНЭ алдааны хариуг л хүлээн авна); (б) ТЭР НЭГ
    // удаагийн flush-ийн ДАРАА ирэх ЯМАР Ч дуудлага бол headers "бодитоор
    // явсан эсэхээс" (жинхэнэ Node.js урсгалд яг энэ) БИШ, зөвхөн `flushed`
    // flag-аас шалтгаалж зүгээр л чимээгүй хаягдана — өөрөөр хэлбэл
    // "сүүлчийн бичилт ялна" зарчим ЗӨВХӨН flush-ийн ӨМНӨХ (ганц удаагийн)
    // цонхонд хамаарна, flush-ийн ДАРАА бол цэвэр "хамгийн ЭХНИЙХ нь
    // мөнхөд ялсан, дараагийн БҮХ дуудлага алгасагдана" горимд шилждэг
    // (`rls.middleware.spec.ts`-ийн "flush АЛЬ ХЭДИЙН болсны ДАРАА..."
    // тестээр хоёр горимыг ялгаатай нь тусад нь баталгаажуулсан).
    let flushed = false;
